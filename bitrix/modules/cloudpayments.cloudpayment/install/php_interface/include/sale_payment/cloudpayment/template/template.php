<?
  ...
  function get_course($PRICE, $CURRENCY, $COURSE_RATE) {
    return $PRICE;
    /*    if ($PRICE && $CURRENCY):
    ...
    if($params['CHECKONLINE'] != 'N') {
      if($status_id != $CLOUD_PARAMS['STATUS_AU']['VALUE'] and $status_id != $CLOUD_PARAMS['STATUS_AUTHORIZE']['VALUE'] and !$paymentCollection->isPaid()) {
        ...
        foreach($basketItems as $basketItem) {
          ...
          if($params['PAYMENT_CURRENCY'] && $params['PAYMENT_CURRENCY'] != "RUB"):
            $PRODUCT_PRICE = get_course($PRODUCT_PRICE, $params['PAYMENT_CURRENCY'], $params['COURSE_RATE']);
          endif;
        ...
        if($order->getDeliveryPrice() > 0 && $order->getField("DELIVERY_ID")) {
          ...
          if($params['PAYMENT_CURRENCY'] && $params['PAYMENT_CURRENCY'] != "RUB"):
            $PRODUCT_PRICE_DELIVERY = get_course($PRODUCT_PRICE_DELIVERY, $params['PAYMENT_CURRENCY'], $params['COURSE_RATE']);
          endif;
