...
$arAllOptions = array(
    array("bonus_percent", 'Процент бонусов за выолненный заказ', "", array("text", "5")),
    array("status_id", 'Статус заказа "Выполнен"', "", array("text", "5")),
    ['partner_comission', 'Комиссия партнеру за выполненный заказ, бонусов', '', ['text', '5']],
);
...
<form method="post"
    ...
    <? if (strlen($_REQUEST["back_url_settings"]) > 0): ?>
        <input type="button" name="Cancel" value="<?= GetMessage("MAIN_OPT_CANCEL") ?>"
               title="<?= GetMessage("MAIN_OPT_CANCEL_TITLE") ?>" on
               click="window.location='<? echo htmlspecialcharsbx(CUtil::addslashes($_REQUEST["back_url_settings"])) ?>'">
        <input type="hidden" name="back_url_settings" value="<?= htmlspecialcharsbx($_REQUEST["back_url_settings"]) ?>">
    <? endif ?>
    <input type="submit" name="RestoreDefaults" title="<? echo GetMessage("MAIN_HINT_RESTORE_DEFAULTS") ?>" On
           Click="return confirm('<? echo AddSlashes(GetMessage("MAIN_HINT_RESTORE_DEFAULTS_WARNING")) ?>')"
